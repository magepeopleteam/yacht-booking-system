...
$cta_disabled      = '1' === get_post_meta( $yacht_id, 'cta_disabled', true );
$cta_eyebrow       = Settings::get( 'cta_eyebrow', __( 'Ready when you are', 'magepeople-yacht-booking-system' ) );
$cta_heading_tpl   = get_post_meta( $yacht_id, 'cta_heading', true ) ?: Settings::get( 'cta_heading', __( 'Book the {yacht_name} today', 'magepeople-yacht-booking-system' ) );
$cta_text          = get_post_meta( $yacht_id, 'cta_text', true ) ?: Settings::get( 'cta_text', __( 'Dates fill fast - lock in your preferred slot with an instant booking request.', 'magepeople-yacht-booking-system' ) );
$cta_button_label  = Settings::get( 'cta_button_label', __( 'Book Now', 'magepeople-yacht-booking-system' ) );
$cta_button2_label = Settings::get( 'cta_button2_label', __( 'Browse all yachts', 'magepeople-yacht-booking-system' ) );
$cta_heading       = strtr( $cta_heading_tpl, array( '{yacht_name}' => get_the_title( $yacht_id ) ) );
	...
	<div class="ybs-ys-container">
			...
			<section class="ybs-ys-gallery" data-ybs-reveal aria-label="<?php esc_attr_e( 'Photo gallery', 'magepeople-yacht-booking-system' ); ?>">
				...
				<div class="ybs-ys-gallery__actions">
					<button type="button" class="ybs-ys-pill-btn" data-ybs-lightbox-open="0">
						...
						<?php echo esc_html( /* translators: %d: number of photos in the gallery. */ sprintf( __( 'View Photos (%d)', 'magepeople-yacht-booking-system' ), count( $gallery ) ) ); ?>
					</button>
				</div>
			</section>
		<?php endif; ?>
			...
			<section class="ybs-ys-specs" data-ybs-reveal aria-label="<?php esc_attr_e( 'Specifications', 'magepeople-yacht-booking-system' ); ?>">
				...
				<?php if ( $length ) : ?>
					<div class="ybs-ys-spec">
						...
						<span class="ybs-ys-spec__value"><?php echo esc_html( /* translators: %s: yacht length in metres. */ sprintf( __( '%s m', 'magepeople-yacht-booking-system' ), $length ) ); ?></span>
					</div>
				<?php endif; ?>
				<?php if ( $cabins ) : ?>
					<div class="ybs-ys-spec">
		...
		<div class="ybs-ys-layout">
			<main class="ybs-ys-main">
				...
				<section class="ybs-ys-section" data-ybs-reveal>
					...
					<h2 class="ybs-ys-h2"><?php echo esc_html( /* translators: %s: yacht name. */ sprintf( __( 'The %s experience', 'magepeople-yacht-booking-system' ), get_the_title( $yacht_id ) ) ); ?></h2>
					<?php
					...
					$ybs_content = apply_filters( 'the_content', get_post_field( 'post_content', $yacht_id ) );
					?>
					<div class="ybs-ys-prose"><?php echo $ybs_content; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- post content passed through the_content, as core does. ?></div>
				</section>
				...
				<?php if ( $occasions ) : ?>
					<section class="ybs-ys-section" data-ybs-reveal>
						...
						<div class="ybs-ys-chips">
							<?php
							foreach ( $occasions as $term ) :
								// get_term_link() returns WP_Error for a term whose
								// taxonomy is gone; passing that to esc_url() is a
								// TypeError on PHP 8.
								$ybs_term_link = get_term_link( $term );
								?>
								<a class="ybs-ys-chip" href="<?php echo esc_url( is_wp_error( $ybs_term_link ) ? '' : $ybs_term_link ); ?>"><?php echo esc_html( $term->name ); ?></a>
							<?php endforeach; ?>
						</div>
					</section>
				<?php endif; ?>
								...
								<?php
									...
									$other_meta    = array_filter(
										array(
											...
											get_post_meta( $other->ID, 'capacity', true ) ? get_post_meta( $other->ID, 'capacity', true ) . __( ' guests', 'magepeople-yacht-booking-system' ) : '',
										)
									);
				...
				<?php if ( is_array( $faq ) && $faq ) : ?>
					<section class="ybs-ys-section" data-ybs-reveal id="ybs-faq">
						...
						<h2 class="ybs-ys-h2"><?php echo esc_html( /* translators: %s: yacht name. */ sprintf( __( 'Questions about the %s', 'magepeople-yacht-booking-system' ), get_the_title( $yacht_id ) ) ); ?></h2>
		...
		<?php if ( ! $cta_disabled ) : ?>
			<section class="ybs-ys-cta" data-ybs-reveal>
				<span class="ybs-ys-eyebrow"><?php echo esc_html( $cta_eyebrow ); ?></span>
				<h2 class="ybs-ys-h2"><?php echo esc_html( $cta_heading ); ?></h2>
				<p><?php echo esc_html( $cta_text ); ?></p>
				<div class="ybs-ys-cta__actions">
					<a class="ybs-ys-btn ybs-ys-btn--gold" href="#ybs-book"><?php echo esc_html( $cta_button_label ); ?></a>
					<a class="ybs-ys-btn ybs-ys-btn--ghost" href="<?php echo esc_url( get_post_type_archive_link( Yacht::POST_TYPE ) ); ?>"><?php echo esc_html( $cta_button2_label ); ?></a>
				</div>
			</section>
		<?php endif; ?>
	</div>
