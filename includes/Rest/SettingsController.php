...
class SettingsController extends Controller {
	...
	public static function install_woocommerce() {
		...
		if ( ! file_exists( WP_PLUGIN_DIR . '/' . $plugin_file ) ) {
			if ( ! current_user_can( 'install_plugins' ) ) {
				return new WP_Error( 'ybs_cannot_install', __( 'You do not have permission to install plugins.', 'yacht-booking-system' ), array( 'status' => 403 ) );
			}
			...
			if ( ! $result ) {
				return new WP_Error( 'ybs_wc_install_failed', __( 'Could not download or extract WooCommerce.', 'yacht-booking-system' ) );
			}
		}

		if ( ! current_user_can( 'activate_plugins' ) ) {
			return new WP_Error( 'ybs_cannot_activate', __( 'You do not have permission to activate plugins.', 'yacht-booking-system' ), array( 'status' => 403 ) );
		}

		self::activate_without_loading( $plugin_file );
	...
	public static function list_woocommerce_gateways() {
		if ( ! class_exists( 'WooCommerce' ) || ! function_exists( 'WC' ) ) {
			return new WP_Error( 'ybs_woocommerce_inactive', __( 'WooCommerce is not active.', 'yacht-booking-system' ), array( 'status' => 400 ) );
		}
	...
	public static function toggle_woocommerce_gateway( WP_REST_Request $request ) {
		...
		if ( ! isset( $gateways[ $gateway_id ] ) ) {
			return new WP_Error( 'ybs_unknown_gateway', __( 'Unknown WooCommerce payment gateway.', 'yacht-booking-system' ), array( 'status' => 404 ) );
		}
